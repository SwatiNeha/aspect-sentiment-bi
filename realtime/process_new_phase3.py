...
# Aspects extractor (KeyBERT + spaCy)
kw_model = KeyBERT()
...
def analyze_sentiment(txt: str, chunk_size=512):
    # Split into chunks of max 512 tokens
    tokens = sentiment_pipeline.tokenizer(
        txt, truncation=False, return_tensors="pt"
    )["input_ids"].squeeze().tolist()

    chunks = [tokens[i:i+chunk_size] for i in range(0, len(tokens), chunk_size)]
    results = []
    for chunk in chunks:
        text_chunk = sentiment_pipeline.tokenizer.decode(chunk, skip_special_tokens=True)
        results.append(sentiment_pipeline(text_chunk, truncation=True, max_length=512)[0])

    # Example: take the chunk with strongest confidence
...
if os.path.exists(MODEL_PATH):
    ...
    if os.path.exists(DB_PATH):
        ...
        if not all_reviews.empty:
            ...
            topic_model.reduce_topics(docs, nr_topics=20)  # force more diversity
...
JUNK_WORDS = {
    ...
    # short tokens
    "nah","yup","wow","ugh","meh","ayy","ehh","yea","ya","ur","u",
