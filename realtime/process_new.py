...
VOCAB_ASPECTS = ["battery","camera","screen","ui","support","shipping","packaging","build","delivery","quality","price"]
...
def main():
    ...
    sess = SessionLocal()
    try:
        rows = sess.query(Review).outerjoin(Processed, Processed.review_id == Review.id)             .filter(Processed.id == None).limit(1000).all()
        if not rows:
...
def export_power_bi_tables(sess: Session):
    df_reviews = pd.read_sql(sqltext("SELECT id as review_id, source, author, text, created_at FROM reviews_raw"), con=engine)
    df_reviews.to_csv("data/processed/reviews_clean.csv", index=False)
    rows = []
    proc = pd.read_sql(sqltext("SELECT review_id, aspect_csv, sentiment_label, score_signed FROM reviews_processed"), con=engine)
    ...
    for _, r in proc.iterrows():
        ...
        for a in aspects:
            if a:
                aspect_sent.append({"review_id": r["review_id"], "aspect": a, "sentiment_label": r["sentiment_label"], "score_signed": r["score_signed"]})
    ...
    joined = proc.merge(df_reviews[["review_id","date"]], left_on="review_id", right_on="review_id", how="left")
    daily = joined.groupby("date").agg(avg_sentiment=("score_signed","mean"), n_reviews=("review_id","count")).reset_index()
    daily.to_csv("data/processed/daily_metrics.csv", index=False)
    import pandas as pd
    pd.DataFrame(columns=["review_id","topic_id","topic_label","topic_prob"]).to_csv("data/processed/topics.csv", index=False)
